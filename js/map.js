...
geolocation.getCurrentPosition(function(r) {
	if(this.getStatus() == BMAP_STATUS_SUCCESS) {
		...
		map.panTo(r.point);
		console.log(r)
		gaddress =  r.address.city + "·" + r.address.district + r.address.street;
		document.getElementById('startLocation').innerHTML =gaddress;
	} else {
...
window.windowMapBusiness = {
		...
		if(addr) {
			...
			geolocation.getCurrentPosition(function(r) { //定位结果对象会传递给r变量 r.point.lng 经度 r.point.lat 纬度
				if(this.getStatus() == BMAP_STATUS_SUCCESS) { //通过Geolocation类的getStatus()可以判断是否成功定位。
					...
					map.centerAndZoom('', s); //设定地图的中心点和坐标并将地图显示在地图容器中
				}
			}, {
				enableHighAccuracy: true
			});
	...
	autoComplete: function(suggestId,el, setLocation, keywords, sc, lf, top) { // id必填 keywords-array 其余-string
		...
		ac.addEventListener("onconfirm", function(e) {
			...
			console.log(myValue);
			console.log(el)
			document.getElementById(el).innerHTML = myValue
			
			document.getElementById('mapBox').style.left = '100%';
